    ...
    private static void searchBook() {
        ...
        for(int i=0;i<book_list.size();i++){
            if(book_list.get(i).b_title.equals(sc)){
                ...
                System.out.printf("  %-25s ||  %-15s||  %-20s||  %-15s||  %-15s||\n","Book Title","Book Author","Book Type","Book Price","Book IN/Out");
        ...
        if(bo_no>3){
            ...
            for(int j=0;j<bo_no;j++){
                if(user_list.get(index).u_amt>500){
                    ...
                    int n = scan.nextInt();
                    for(int i=0;i<book_list.size();i++){
                        if(book_list.get(i).b_id==n && book_list.get(i).b_count<1){bor_book.add(newborrow(book_list.get(i).b_title,book_list.get(i).b_auth,book_list.get(i).b_type,
 book_list.get(i).b_price,book_list.get(i).b_id,book_list.get(i).b_count,user_list.get(index).u_id));
    ...
    private static void borrow_history() {
        ...
        for(int i=0;i<bor_book.size();i++){
            if(user_list.get(index).u_id==bor_book.get(i).u_id){
                System.out.printf("  %-26s ||  %-15s||  %-20s||  %-15s|| 15s||\n",bor_book.get(i).b_title,bor_book.get(i).b_auth,bor_book.get(i).b_type,bor_book.get(i).b_id,bor_book.get(i).b_price,bor_book.get(i).u_id);
                f++;
            }
        }
        if(f==0){
    ...
    private static void books(){
        ...
        book_list.add(new book("The Adventures of Tow","Mark Twain","Novel",200,101,0));
...
class users  extends LibrarySystem{
...
class borrow extends LibrarySystem{
    String b_title,b_auth,b_type;
